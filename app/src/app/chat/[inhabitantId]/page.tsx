...
import { fetcher, type InhabitantInfo } from "@/lib/api";
...
export default function ChatInhabitantPage() {
  const params = useParams();
  const inhabitantId = params.inhabitantId as string;

  const scrollRef = useRef<HTMLDivElement>(null);

  const { data: configData } = useSWR<InhabitantInfo>(
    `/api/inhabitants/${inhabitantId}/config`,
    fetcher,
  );

  const { data, isLoading, mutate } = useSWR<MessagesResponse>(
    `/api/inhabitants/${inhabitantId}/chat/messages`,
    fetcher,
    {
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      try {
        await fetch(`${API_BASE}/api/inhabitants/${inhabitantId}/chat/messages`, {
          method: "POST",
      ...
      <header className="sticky top-0 z-10 border-b backdrop-blur-md" style={{
        ...
        <div className="mx-auto max-w-4xl px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            >
              {configData?.displayName || configData?.name || "Chat"}
            </h1>
