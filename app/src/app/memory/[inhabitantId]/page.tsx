...
export default function MemoryInhabitantPage() {
  ...
  const { data: filesData, isLoading: filesLoading } =
    useSWR<MemoryFilesResponse>(`/api/inhabitants/${inhabitantId}/memory`, fetcher);

  const { data: fileContent, isLoading: contentLoading } =
    useSWR<MemoryFileContent>(
      selectedFile ? `/api/inhabitants/${inhabitantId}/memory/${selectedFile}` : null,
      fetcher
    );
